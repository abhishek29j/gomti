...
async function sendLeadViaSmtp(details: string, smtpConfig: { host: string; port: number; secure: boolean; user: string; pass: string }, fromAddress: string, targetEmail: string) {
    ...
    if (smtpConfigured) {
      ...
      try {
        await sendLeadViaSmtp(
          details,
          { host: smtpHost, port: smtpPort as number, secure: smtpSecure, user: smtpUser, pass: smtpPass },
          fromAddress,
          targetEmail,
        );
        smtpSuccess = true;
